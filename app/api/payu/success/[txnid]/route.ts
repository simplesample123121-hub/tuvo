...
export async function POST(request: NextRequest, context: { params: Promise<{ txnid: string }> }) {
  try {
    ...
    return NextResponse.redirect(`${origin}/payment/success/${txnid}?${search.toString()}`, { status: 302 })
  } catch {
    const origin = new URL(request.url).origin
    const { txnid } = await context.params
    return NextResponse.redirect(`${origin}/payment/success/${txnid}`, { status: 302 })
  }
}
